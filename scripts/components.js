    ...
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700/60 overflow-hidden">
      <div className="px-4 py-3 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          {series.length > 0 && onOpenSqlEditor && (
            <EditSqlButton hasCustomSql={hasCustomSql} onClick={onOpenSqlEditor} />
          )}
          {canRemovePlot && (
            <button
...
const SqlEditorModal = ({ isOpen, sql, error, loading, onClose, onApply, onReset, onChange }) => {
  if (!isOpen) return null;

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-2xl border border-gray-200 dark:border-gray-700 w-full max-w-6xl mx-4 max-h-[95vh] flex flex-col">
        ...
        <div className="flex-1 p-4 overflow-auto">
          <p className="text-sm text-gray-600 dark:text-gray-400 mb-3">
            Edit the SQL query to add custom filters or transformations.
            Use <code className="px-1 py-0.5 bg-gray-200 dark:bg-gray-700 rounded text-xs font-mono">{"{{START_TIME}}"}</code> and <code className="px-1 py-0.5 bg-gray-200 dark:bg-gray-700 rounded text-xs font-mono">{"{{END_TIME}}"}</code> as
            placeholders for the current view range (updated during zoom/pan).
          </p>
          <textarea
            value={sql}
            onChange={(e) => onChange(e.target.value)}
            className="w-full h-[32rem] font-mono text-sm bg-gray-50 dark:bg-gray-900 border border-gray-300 dark:border-gray-600 rounded-lg p-3 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none"
            placeholder="SELECT ..."
            spellCheck={false}
          />
          {error && (
            <div className="mt-3 p-3 bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 rounded-lg">
              ...
              <p className="text-xs text-red-600 dark:text-red-400 mt-1 font-mono whitespace-pre-wrap">{error}</p>
            </div>
          )}
        </div>
        ...
        <div className="flex items-center justify-between px-4 py-3 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/50">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              {loading && <div className="spinner" style={{ width: 14, height: 14, borderWidth: 2 }}></div>}
              Apply
            </button>
          </div>
        </div>
...
const EditSqlButton = ({ hasCustomSql, onClick }) => (
  <button
    onClick={onClick}
    className="relative text-xs px-2 py-1 rounded bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 border border-gray-300 dark:border-gray-600 text-gray-900 dark:text-gray-200 flex items-center gap-1"
    title={hasCustomSql ? "Edit custom SQL (active)" : "Edit SQL query"}
  >
    ...
    SQL
    {hasCustomSql && (
      <span className="absolute -top-1 -right-1 w-2.5 h-2.5 bg-yellow-400 rounded-full border border-white dark:border-gray-800"></span>
    )}
  </button>
);
